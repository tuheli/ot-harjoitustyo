...
class Menu:
    def __init__(self, screen: pygame.Surface, load_game_callback, set_active_tilemap_path_callback, load_editor_callback) -> None:
        ...
        self.title = GlowText(title_text, title_font,
                              COLOR_TEXT, COLOR_TEXT_GLOW, title_position)
        ...
        for i in range(4):
            ...
            text = ''
            if i == 0:
                def on_click(): return self.on_click_button(DEFAULT_TILEMAP_PATH)
                text = f'Level {i + 1}'
            else:
                def on_click(): return None
        ...
        self.right_side_lines = self.create_faded_lines(
            'right', screen.get_width() - line_x)
    ...
    def create_faded_lines(self, direction, start_x):
        ...
        for _ in range(4):
            ...
            if direction == 'right':
                ...
                line_color = (
                    line_color[0], line_color[1] - green_fade_strength, line_color[2] - blue_fade_strength)
            else:
                line_start_x -= line_position_offset
                line_color = (
                    line_color[0], line_color[1] - green_fade_strength, line_color[2] - blue_fade_strength)
